...
<section>
    ...
    <table>
      ...
      <tr>
	      <c:if test="${ndto != null}">
    	    <td colspan="3"><strong>다음글</strong> <span class="separator">|</span><a href="/board/bview?bno=${ndto.bno}">${ndto.btitle}</a></td>
  	    </c:if>
	      <c:if test="${ndto == null}">
    	    <td colspan="3"><strong>다음글</strong> <span class="separator">|</span>다음 글이 없습니다.</td>
  	    </c:if>
      </tr>
      <tr>
	      <c:if test="${pdto != null}">
	        <td colspan="3"><strong>이전글</strong> <span class="separator">|</span><a href="/board/bview?bno=${pdto.bno}">${pdto.btitle}</a></td>
  	    </c:if>
	      <c:if test="${pdto == null}">
	        <td colspan="3"><strong>이전글</strong> <span class="separator">|</span>이전 글이 없습니다.</td>
  	    </c:if>
      </tr>
    </table>
